...
```bash
# Quick health status
curl -H "x-api-key: $API_KEY" http://localhost:3005/api/status?action=health
...
curl -H "x-api-key: $API_KEY" http://localhost:3005/api/status?action=overview
```
...
```bash
curl -H "x-api-key: $API_KEY" http://localhost:3005/api/releases/check
...
curl -X POST -H "x-api-key: $API_KEY" http://localhost:3005/api/releases/update
```

### Database Backup

```bash
curl -X POST -H "x-api-key: $API_KEY" http://localhost:3005/api/backup
```
...
```bash
# List agents
curl -H "x-api-key: $API_KEY" http://localhost:3005/api/agents

# Register an agent
curl -X POST -H "x-api-key: $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"name": "my-agent", "type": "openclaw"}' \
  http://localhost:3005/api/agents
```
...
STATUS=$(curl -sf -H "x-api-key: $API_KEY" http://localhost:3005/api/status?action=health | jq -r '.status')
