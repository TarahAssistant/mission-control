...
app.prepare().then(() => {
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    // Build the upgrade request to the gateway, forwarding ALL browser headers
    ...
    // Unconditionally override Origin to ensure gateway ALWAYS treats it as Control UI
    ...
    proxyReq.on('upgrade', (proxyRes, proxySocket, proxyHead) => {
      // Send the 101 Switching Protocols back to the browser
      ...
      socket.on('error', (err) => { console.error(`[ws-proxy] browser socket error: ${err.message}, browser sent ${browserFrames} frames, gateway sent ${gatewayFrames} frames`); proxySocket.destroy() })
      proxySocket.on('close', () => { console.log(`[ws-proxy] gateway closed. browser=${browserFrames} gateway=${gatewayFrames} frames`); socket.destroy() })
      socket.on('close', () => { console.log(`[ws-proxy] browser closed. browser=${browserFrames} gateway=${gatewayFrames} frames`); proxySocket.destroy() })
    })
    ...
    // If the gateway responds with a non-101 (e.g., 403), forward the error
